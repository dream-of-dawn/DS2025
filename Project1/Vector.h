...
public:
	...
	void selectionSort() { selectionSort(0, _size); }
	...
	void quickSort() { quickSort(0, _size); }
...
template <typename T> void Vector<T>::shrink() {
	...
	for (int i = 0; i < _size; i++)_elem[i] = oldElm[i]; // 复制原有元素到新空间
...
template <typename T> void Vector<T>::quickSort(Rank lo, Rank hi) {
	if (hi - lo < 2) return; // 区间长度小于2时已有序，直接返回
	Rank pivot = partition(lo, hi); // 划分操作，返回枢轴位置
	quickSort(lo, pivot); // 递归排序左子区间
	quickSort(pivot + 1, hi); // 递归排序右子区间
}
//快速排序的划分操作
template <typename T> Rank Vector<T>::partition(Rank lo, Rank hi) {
	T pivot = _elem[lo]; // 选择区间第一个元素作为枢轴
	Rank i = lo + 1; // i指向当前扫描位置
	Rank j = hi - 1; // j指向区间末尾位置
	while (true) {
		while (i <= j && _elem[i] <= pivot) i++;// 从左向右扫描，找到大于枢轴的元素
		while (i <= j && _elem[j] >= pivot) j--; // 从右向左扫描，找到小于枢轴的元素
		if (i > j) break; // i和j交叉，划分完成
		swap(_elem[i], _elem[j]); // 交换i和j位置的元素
	}
	swap(_elem[lo], _elem[j]); // 将枢轴放到正确位置
	return j; // 返回枢轴位置
}
